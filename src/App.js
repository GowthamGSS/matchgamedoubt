...
class App extends Component {
  ...
  imageClick = id => {
    ...
    if (id === imagesList[imgIndx].id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgIndx: Math.floor(Math.random() * imagesList.length),
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({
  ...
  render() {
    const {timer, tabId, imgIndx, score, isOut} = this.state
        ...
        <div className="appContainer">
          ...
          {!isOut && (
            <div className="imgCard">
              <img
                src={imagesList[imgIndx].imageUrl}
                className="bigImg"
                alt="match"
              />
              <ul className="tabContainer">
                {tabsList.map(ele => (
                  <TabItem key={ele.tabId} ele={ele} TabClick={this.tabClick} />
                ))}
              </ul>
              <ul className="imageList">
                {imagesList
                  .filter(ele => ele.category === tabId)
                  .map(elem => (
                    <ImageItem
                      key={elem.id}
                      elem={elem}
          ...
          {isOut && (
            <ScoreCard score={score} btnClick={this.btnClick} timer={timer} />
          )}
        </div>
      </div>
    )
